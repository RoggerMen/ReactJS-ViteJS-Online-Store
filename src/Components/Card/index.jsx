...
const Card = ({item}) => {
  ...
  const renderIcon = (id) => {
    // ENTONCES SI ESE PRODUCTO ESTA DENTRO DE CART ENTONCES EL NOS VA A DECIR COMO TRUE
    // SI EL TAMAÑO ES MAYOR A 0 ES PORQUE YA ESTA EN EL CARRITO
    // ENTONCES EL VALOR DE isInCart es un "booleano"
    const isInCart = context.cartProducts.filter(product => product.id === id).length > 0;

    if (isInCart) {
      return (
        <button className="absolute top-0 right-0 flex justify-center items-center bg-green-600 w-6 h-6 rounded-full m-2 p-1">
